...
{ 
	...
	//COLORREF col = RGB(240, 240, 24);

	//P2N(_T("Getfocus%p this%p\r\n"), GetFocus(), this);
...
//	pDC->GetTextMetrics(&tm);							// get font metrics
	...
	if (lpDrawItemStruct->itemState & ODS_DISABLED){	// DISABLED BUTTON
		...
		CRect rct(0,0,0,0);	pDC->DrawText(sCaption,&rct, DT_CALCRECT);
		CRect rct2(tr); rct2.top = (tr.bottom - rct.bottom) / 2;
		pDC->DrawText(sCaption, &rct2, m_TextAlign);		
...
//---------------------------------------------------------------------------
            ...
            if (m_tracking)
				...
				COLORREF bcol = maincolor;
				int rr = (bcol) & 0xff;
				int gg = (bcol >> 8) & 0xff;
				int bb = (bcol >> 16) & 0xff;

				rr = 12 * rr / 8;	rr = min(rr, 255);
				gg = 12 * gg / 8;	gg = min(gg, 255);
				bb = 12 * bb / 8;	bb = min(bb, 255);
...
bool CxTargetButton::SetFont(CString sFontName,long lSize, long lWeight, BYTE bItalic, BYTE bUnderline)
{
	...
	if (m_pLF){
		...
		_tcsncpy_s(m_pLF->lfFaceName, sizeof(m_pLF->lfFaceName), sFontName,31);
...
void	CxTargetButton::SetColor(int col)
...
void CxTargetButton::DrawFocus(CDC *pDC, RECT *r, DWORD color, int border, int width, int height, int offset)
