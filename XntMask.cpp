...
/* =====[ xntmask.cpp ]========================================== 
   ...
   Description:     The xraynotes project, implementation of the xntmask.cpp                
...
// XntMask.cpp: implementation of the XntMask class.
//
//////////////////////////////////////////////////////////////////////
...
DWORD	XntMask::ROP3FromIndex(DWORD dwIndex)
...
BYTE	XntMask::SwapROP3_SrcDst(BYTE bRop3)

{
    ...
    bits.bit1 = bits.bit2;  bits.bit2 = t;

    t = bits.bit5;
    bits.bit5 = bits.bit6;  bits.bit6 = t;
...
HBITMAP	XntMask::MakeMaskBmp(HBITMAP* pBmpSource, COLORREF TransCol, 
								int iTransPixX,  int iTransPixY)

{
	HBITMAP	ret = 0;

	MakeMaskBmp(pBmpSource, &ret, TransCol, iTransPixX, iTransPixY);
...
void XntMask::MakeMaskBmp(HBITMAP* pBmpSource, HBITMAP* hBmpMask,
								...
								int iTransPixelX,  int iTransPixelY  )
{
...
#if 0
	TCHAR str[255]; _snprintf(str, sizeof(str), 
	...
	HDC hdcSrc = CreateCompatibleDC(GetDC(HWND_DESKTOP));	
	////ASSERT(hdcSrc);
	HDC hdcDst = CreateCompatibleDC(GetDC(HWND_DESKTOP));	
	////ASSERT(hdcDst);

	*hBmpMask = CreateBitmap(ww, hh, 1, 1, NULL); 
	////ASSERT(*hBmpMask);

	// Load the bitmaps into memory DC
	HBITMAP hbmold1 =  (HBITMAP) SelectObject(hdcSrc, *pBmpSource); 
	////ASSERT(hbmold1);
	HBITMAP hbmold2 =  (HBITMAP) SelectObject(hdcDst, *hBmpMask);	
	////ASSERT(hbmold2);
...
HBITMAP XntMask::MakeMaskBmpDC(HDC hdcSrc, int ww, int hh, COLORREF clrTrans)
							
{
	...
	HDC hdcDst = CreateCompatibleDC(GetDC(HWND_DESKTOP));	
	////ASSERT(hdcDst);
	HBITMAP hBmpMask = CreateBitmap(ww, hh, 1, 1, NULL); 
	////ASSERT(hBmpMask);
...
BOOL	XntMask::MaskBlt(HDC hdcDest, int nXDest, int nYDest, 
				int nWidth, int nHeight,
					HDC hdcSrc, int nXSrc, int nYSrc,
						HBITMAP hbmMask, int xMask, int yMask,
							 DWORD dwRop, int alpha)
{
	//P2N(_T("XntMask::XntMaskBlt %d %d\r\n"), nWidth, nHeight);

	if ( hbmMask == NULL )
		{
		BOOL ret;
		//P2N(_T("XntMaskBlt NULL mask\r\n"));

		if(alpha == 255)
			{
			ret = BitBlt(hdcDest, nXDest, nYDest, nWidth, nHeight, 
							hdcSrc, nXSrc, nYSrc, SRCCOPY);
			}
		else
			{
			BLENDFUNCTION m_bf;	INITBLEND(m_bf, alpha);
			ret = AlphaBlend(hdcDest, nXDest,nYDest, nWidth, nHeight, 
						hdcSrc, nXSrc, nYSrc, nWidth, nHeight, m_bf); 	
			}
		return ret;
		}
	...
	if(alpha == 255)
		...
		{
		BLENDFUNCTION m_bf;	INITBLEND(m_bf, alpha);
		VERIFY(AlphaBlend(hdcDest, nXDest,nYDest, nWidth, nHeight, hDC1, 0, 0, nWidth, nHeight, m_bf)); 	
		}
	...
	DeleteDC(hDC1);	DeleteDC(hDC2);	DeleteDC(hDCMask);
