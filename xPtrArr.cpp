...

{
	...
	for(int loop = size - 1; loop >= 0; loop--)
		...
		if(GetAt(loop) == ptr)
			{	
			//P2N(_T("CxPtrArr::RemovePtr removed %p\r\n"), ptr);
			RemoveAt(loop);
			}
		}
}
...

{
	...
	if(!was)
		{
		//P2N(_T("CxPtrArr::AddUniq added %p\r\n"), ptr);
		ret = Add((void*)ptr);
		}	
	return ret;
}
...

{
	...
	//P2N(_T("CxPtrArr::SetAtGrow index=%d\r\n"), nIndex);
...
{	
	...
	if(nIndex + nCount > m_nSize)
		{
		P2N(_T("CxPtrArr::RemoveAt Removing from past the end nIndex=%d m_nSize=%d\r\n"), nIndex, m_nSize);
		return;
		}

	//P2N(_T("CxPtrArr::RemoveAt index=%d\r\n"), nIndex);
...
{
	P2N(
	_T("CxPtrArr::Dump data=%p m_nSize=%d m_nMaxSize=%d m_nGrowBy=%d\r\n"),
