...
public:
	...
	void	encrypt(char *buff, int blen, const TCHAR *pass, int plen);
	void	decrypt(char *str, int slen, const TCHAR *pass, int plen);
	ulong   hash(const char *buff, int blen);
	ulong   crypthash(const char *buff, int blen, TCHAR  *pass, int plen);

	#ifdef DEF_DUMPHEX
	TCHAR 	*bluepoint_dumphex(const char *str, int len);
	#endif

	TCHAR    *tohex(const char *str, int len, TCHAR *out, int *olen , int llen = 0);
	char    *fromhex(const TCHAR *str, int len, char *out, int *olen);

protected:

	void    do_encrypt(char *str, int slen, TCHAR *pass, int plen);
	void    do_decrypt(char *str, int slen, TCHAR *pass, int plen);
	void    prep_pass(const TCHAR *pass, int plen, TCHAR *newpass);

};

extern CBluePointA bluepointa;
